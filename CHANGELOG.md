...
- **Choose the document font (PR #119, contributed by [@Pat9496](https://github.com/Pat9496)).** **View → Font** opens a searchable picker of every installed system font family, with a pinned System Default row; the choice persists across restarts, and an unknown family falls back to auto-detection with a logged warning instead of failing. A scanned-fonts cache and a precomputed lowercase copy keep the filter allocation-free while it is open. The same PR adds **View → Smooth Text Rendering**, which turns off egui's text pixel-rounding for readers who prefer smoother glyph edges over crisp ones.

- **Fragment links land on the heading they name (#141).** `guide.md#section` opened the document and dropped the anchor; same-document `#section` links did nothing at all. Both now navigate: the fragment is matched against the document's headings using the GitHub anchor convention (`#settings--config` matches a `Settings & Config` heading, and the first `h1` anchors to the top of the page), and the tab lands on it through the same cached-position-plus-corrective flow outline clicks use — so image-heavy documents land just as accurately. Ctrl+click carries the fragment into the new tab, and opening an already-open document at a fragment focuses the tab *and* scrolls instead of silently dropping the anchor. Unmatched fragments stay no-ops rather than errors, and classification is untouched: `guide.md?v=2#s` still names `guide.md`, and `%23` remains a literal hash.

### Bug Fixes

- **A scroll offset that outruns the content extent no longer paints a blank frame (#140).** The candidate path survived every previous attempt at evidence: fixtures built to shrink the document under a deep scroll never stranded the offset, the runtime probes reported zero `OFFSET>EXTENT` frames across thousands of instrumented frames, and the screenshot guards sample settled states — a one-frame artifact was outside their reach by construction. An in-process regression test now drives single frames and sees it directly: store an offset past the measured extent, render one frame, and the document clip paints nothing. The mechanism is the render ordering: egui's `show_viewport` computes the viewport window from the stored offset unclamped, the slice is selected for that window, and the scroll clamp only ran after `show_viewport` returned — correcting the *next* frame. The fix clamps a persisted or requested offset that starts beyond the content end *before* viewport selection, snapping it to the deepest scrollable position; near-bottom offsets are untouched, so normal scrolling at the end of a document never fights the scrollbar allowance, and egui's own post-layout clamp remains in place. With the fix the same frame paints 49 text shapes where it previously painted none.
...
- **The vendored renderer is at 0.28.2.** Carries the list-marker alignment fix above plus the scroll-overshoot pre-selection clamp, both landed on main before this tag so `check-fork-publishable` passes and the release's `publish-crates` job can publish the fork before md-viewer — which also restores crates.io parity after 0.2.1 shipped to the other channels without a crates.io upload.
...
- **The divider and its neighbouring scrollbar stop flickering under a stationary pointer (#139, PR #194, hypothesis and fix by [@RichardCao](https://github.com/RichardCao)).** egui's floating scrollbar animates its width between 2 px and 10 px on hover, and a dormant floating bar is hidden by opacity rather than by being thin — so that width governed the bar's *interact* rect and nothing visible. Beside a panel divider the growing and shrinking rect claimed and released the pointer on alternating frames, and at the one pixel just outside the resize handle the two never settled. Pinning the width to a constant removes it. Measured on a virtual display with the pointer held still: on the previous build exactly one x position cycles through four states in both the painted pixels and the cursor shape, with its neighbours stable across twenty frames; with the fix, none. `scripts/hover-stability.sh` is that measurement, checked in as a third manual guard — it covers what the two screenshot guards cannot, because they sample settled states and an oscillation never settles.

- **Dense tables trade a little horizontal overflow for much less height (#118 Phase 2, PR #182, contributed by [@RichardCao](https://github.com/RichardCao)).** When a table's header-word floors already exceed the visible column budget — so sideways scrolling is unavoidable regardless — the renderer now scores a small bounded set of wider layouts and picks the narrowest one that captures 90 % of the available row-height reduction. The added width is capped at `min(160 px, 30 % of the floor total, natural total − floor total)`, and the horizontal viewport is unchanged, so the extra width is scrollable overflow rather than a wider reading column. Below that threshold nothing changes. Every candidate in the set is scored with no early exit, because the height curve is not monotonic: a fixture whose curve dips and recovers is checked in, so a later reader who assumes otherwise gets a failing test rather than a plausible-looking result.

### Bug Fixes

- **A link whose destination cannot be decoded is reported instead of doing nothing (#141, PR #191).** A destination whose percent escapes are not valid UTF-8 — `bad%FF.md` — passed the check that decides which links are local, so it rendered as an internal link, and clicking it produced no navigation and no error whatsoever. It now reports through the same error bar as a missing file. Two neighbouring inconsistencies went with it: a protocol-relative destination such as `//host/path` was read as an absolute filesystem path and treated as a local Markdown link, and is now external; and a query string was kept in the file name by the resolver while the link check rejected it, so `guide.md?v=2` is now stripped like a fragment and both agree. The check and the resolver share one classification, so they can no longer disagree — which is what made the first defect possible.
...
- **A plain `cargo test` works in the renderer workspace again (PR #185).** `commonmark!` and `commonmark_str!` exist only under the `macros` feature, but no `[[example]]` section declared `required-features`, so Cargo built `examples/macros.rs` and `examples/mixing.rs` in every configuration and the command aborted with 16 `cannot find macro` errors before running a single test — a failure that reads like a broken checkout rather than a missing flag, and the cost issue #122 attributed to the two-workspace split. The renderer suite now runs with no feature list at all: 0 tests to 148. The same defect in a crate-level doctest is gated on the feature rather than marked `ignore`, because rustdoc compiles a doctest with the crate's own cfgs, so the check survives where the macro exists.
...
- **Fitted table columns are optimized for wrapped height (#118, PR #157, contributed by [@RichardCao](https://github.com/RichardCao)).** The allocator was fair but height-blind: it never asked whether another few pixels for a column would remove a wrapped line. A bounded deterministic search now refines the initial widths above the existing word floors, measuring real row heights — capped at 4096 cells and eight passes, cached by content, bound, typography, pixel scale and math scale. On a six-column prose table the table shrank from 647 px to 573 px, with the first row going from five lines to four. Header word floors are untouched, so `Required` still cannot break mid-word, and dense tables where every column already sits at its floor are deliberately left for a separate policy decision.
...
- **Non-`file:` URI schemes are no longer treated as filesystem paths (#141, PR #154, contributed by [@RichardCao](https://github.com/RichardCao)).** The classifier was a denylist — `http`, `https`, `mailto`, `tel`, `ftp` — so anything else ending in `.md`, `.markdown` or `.txt` fell through to path handling: `data:text/plain,x.md`, `javascript:void(0)/x.md` and `unknownscheme://host/a.md` were all registered as local links. Nothing was ever executed; resolution simply failed, so such a link was **swallowed** — neither navigated nor handed to the browser. Schemes are now recognized by the RFC 3986 grammar and only scheme-less paths and `file:` URIs are treated as local, with Windows drive letters kept as paths.
...
- **The cross-platform check builds test targets too (PR #155).** `cargo check --locked` compiles only the default targets, so a test behind `#[cfg(target_os = "windows")]` was not merely unexecuted on Windows — it was never compiled anywhere. #154's drive-letter assertion guards the one branch Linux cannot exercise and had nothing building it. `--all-targets` closes that; still `check` rather than `test`, since running the suite on three platforms is a separate decision.
