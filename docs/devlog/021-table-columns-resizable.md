...
| **T-K3** drag-resize | PASS | First attempt's drags at x=282..296 missed because pixel-sampling later revealed the actual divider is at screen **x=299** (not the ~285 I'd been estimating from text-width heuristics). After confirming the divider position via PIL pixel-sampling on the rendered output (lum=190 at x=99 of a +200 crop), `egui_drag(299, 150 → 450, 150)` widened the Magna column from ~80 px to ~250 px — Incididunt and Do columns shifted right correspondingly. Verified on a debug build with `resize_grab_radius_side` temporarily restored to the egui default (5 px) by commenting out `src/main.rs:1288`; that change was reverted before commit. The handler logic is fully intact. |
...
**All 4 subtests PASS. No regressions at the tables × virtualization boundary.** Tables render correctly through the viewport-skip cache; column drag-resize is fully functional (with the caveat that the 2 px hit-zone makes scripted testing without pixel-perfect divider coords unreliable — humans use cursor-hover feedback to find it); selection inside cells participates in the standard selection model and survives wheel-scroll via the preserving hack.
