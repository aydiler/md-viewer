...
### CentralPanel content is clipped to the whole window — wide-block carve-outs need their own pane bound
**Context:** Users could drag wide tables (and saw code blocks) painting **over the right sidebar**, and resizing any sidebar reset every table's manually resized column widths. Two unrelated-seeming symptoms, one egui asymmetry plus one coarse policy.

**Root cause (the clip asymmetry):** egui 0.33 clips `SidePanel`s to their own rect but `CentralPanel` content to `ctx.content_rect()` — the entire window (`egui-0.33.3 containers/panel.rs`: `panel_ui.set_clip_rect(panel_rect)` vs `panel_ui.set_clip_rect(ctx.content_rect())`). `Ui::new_child` **clones the parent painter**, so every descendant inherits that window-wide clip; an explicit child `max_rect` is *not* intersected with it (the renderer's #64 carve-out depends on this). Panels paint in call order, so outline-then-central means central content drawn past its max_rect lands **on top of** the sidebar. Prose never exceeds the pane — only horizontal scrollers (tables, code blocks) ever did.

**Fix:** every carve-out is capped at the scroll viewport's right edge, `ui.clip_rect().right()` — pane-bounded because the renderer-owned vertical ScrollArea sets the content clip to its inner rect. Applied at four sites: the bootstrap/full-render document column, the viewport-slice re-anchor (`content_left + recorded width` used to re-extend past the pane whenever the geometry carries a left offset), both table scope rects, and the code-block frame fill. Residual bleed is egui's ±3px `clip_rect_margin` per nested scroll (≤ ~6px), absorbed by the app's content gutter.

**Root cause (the reset):** `TableBuilder` persists user column widths and ignores later `Column::initial` values; the old `table_layout_bound_changed` reset them on *any* bound change — including every sidebar pixel-nudge. But egui_extras column **drags do not redistribute** width (one column changes by the pointer delta; neighbours are untouched), so the width sum can overflow the bound *by choice* — any "reset when widths overflow" rule fights the user mid-drag.

**Fix:** reset only when the bound **shrank** *and* the persisted widths (shadowed each frame from `body.widths()`) no longer fit it. Growth keeps the layout (`auto_shrink` closes the trailing gap); the bound history is what separates a drag from a genuine shrink.

**Testing technique:** the pane-clip fixture must render through `show_scrollable`, not `.show()` — the renderer-owned ScrollArea is what creates the pane-bounded clip the cap uses; the non-scrollable entry never reproduces the leak. Assert `min(shape.rect.right, shape.clip_rect.right)` (the widest *visibly paintable* pixel) against `pane_right + 8`, and include a guard that the fixture's content actually reaches the pane edge — a cap test whose fixture never approaches the cap passes vacuously. The default `Context::default()` test surface is enormous, so constrain the pane with an explicit `UiBuilder::max_rect` scope, not `set_max_width`.

**Files:** `crates/egui_commonmark/egui_commonmark/src/parsers/pulldown.rs` (`show()` column cap, viewport-slice cap, both table carve-outs, `table_bound_shrink_discards_widths`, `store_table_column_widths`), `crates/egui_commonmark/egui_commonmark_backend/src/elements.rs` (`code_block` width clamp), `crates/egui_commonmark/egui_commonmark/tests/pane_clip.rs`, `docs/devlog/068-wide-block-pane-clip.md`
