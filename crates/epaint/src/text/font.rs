...
pub struct UvRect {
    ...
    /// Whether this glyph's atlas pixels carry real color (e.g. a color-emoji
    /// bitmap). Colored glyphs are tessellated with a white vertex color so
    /// the atlas RGB reaches the screen untinted.
...
pub struct FontImpl {
    ...
    /// Raw font bytes kept around for color-glyph rasterization with swash
...
impl FontImpl {
    ...
    /// Rasterize a color glyph (CBDT/CBLC bitmap, sbix, or COLR layered
    /// outline) with swash and write RGBA pixels into the atlas.
    ///
    /// Returns `None` if the font has no color data for this glyph.
    fn color_glyph_uv_rect(
        &self,
    ...
    pub fn allocate_glyph(
        ...
        // Color-bitmap fonts (CBDT/CBLC, sbix) and COLR fonts have no vector
        // outlines for ab_glyph — the outline path above yields nothing and
        // the glyph would render blank. Fall back to swash, which can scale
        // embedded color bitmaps, and store real RGBA pixels in the atlas.
        let uv_rect = uv_rect.unwrap_or_else(|| {
            color_glyph_uv_rect(
                &self.ab_glyph_font,
                &self.raw_bytes,
                self.font_index,
...
/// Rasterize a color glyph (CBDT/CBLC bitmap, sbix, or COLR layered outline)
/// with swash and write RGBA pixels into the atlas.
///
/// Returns `None` if the font has no color data for this glyph.
///
/// Free function (not a method) so callers inside `allocate_glyph` can borrow
/// the font fields while the glyph-allocation cache holds a mutable borrow.
...
fn color_glyph_uv_rect(
    ...
    let font = swash::FontRef::from_index(raw_bytes, font_index as usize)?;
    // px_scale_factor translates unscaled units to pixels, so multiplying by
    // units_per_em recovers the pixel-per-em size for swash.
    let px_size = metrics.px_scale_factor * ab_glyph_font.units_per_em().unwrap_or(2048.0);
    ...
    let image = Render::new(&sources).render(&mut scaler, glyph_id.0)?;
    ...
    let (glyph_pos, atlas_image) = atlas.allocate((w, h));
    for y in 0..h {
        for x in 0..w {
            let i = (y * w + x) * 4;
            if i + 3 < image.data.len() {
                atlas_image[(glyph_pos.0 + x, glyph_pos.1 + y)] =
                    crate::Color32::from_rgba_unmultiplied(
                        image.data[i],
                        image.data[i + 1],
                        image.data[i + 2],
                        image.data[i + 3],
    ...
    // swash's placement is relative to the baseline, top being positive
    // upwards — same convention as ab_glyph's bounding-box min.
    let offset_in_pixels = vec2(placement.left as f32 + bin.as_float(), -(placement.top as f32));
    let offset =
        offset_in_pixels / metrics.pixels_per_point + metrics.y_offset_in_points * Vec2::Y;
    Some(UvRect {
        offset,
        size: vec2(w as f32, h as f32) / metrics.pixels_per_point,
        min: [glyph_pos.0 as u16, glyph_pos.1 as u16],
