...
impl FontsImpl {
    ...
    pub fn new(
        ...
        for (name, font_data) in &definitions.font_data {
            ...
            // Keep a shared copy of the raw bytes for color-glyph rasterization
            // with swash (see FontImpl::color_glyph_uv_rect).
            let raw_bytes: std::sync::Arc<[u8]> = std::sync::Arc::from(font_data.font.as_ref());
